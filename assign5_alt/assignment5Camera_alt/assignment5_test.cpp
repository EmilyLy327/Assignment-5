...
int main()
{
    ...
    namedWindow("ECE484W_Assignment_5_alt");

    int brightness;         // Declare brightness variable for camera feed
    int contrast;           // Declare contrast variable for camera feed
    
    /*Added Alt*/
    int overlayBrightness;  // Declare brightness variable for overlay image
    int overlayContrast;    // Declare contrast variable for overlay image
    /*Added Alt*/
    ...
    while (1) {
        ...
        // Read brightness value for camera feed from the file and convert to integer
        getline(parametersFromFile, line);
        brightness = stoi(line);

        // Read contrast value for camera feed from the file and convert to integer
        getline(parametersFromFile, line);
        contrast = stoi(line);

        /*Added Alt*/
        // Read brightness value for overlay image from the file and convert to integer
        getline(parametersFromFile, line);
        overlayBrightness = stoi(line);

        // Read contrast value for overlay image from the file and convert to integer
        getline(parametersFromFile, line);
        overlayContrast = stoi(line);
        /*Added Alt*/
        ...
        /*Added Alt*/
        // Convert the contrast and brightness back to original values for overlay image
        minimumValue = 0;
        maximumValue = 511 - 511.0 / 99.0 * overlayContrast; 
        brightnessMultiply = 2.0 / 99.0 * overlayBrightness;
        Mat overlayOutputImage = overlayedImage.clone();

        Mat overlayPixelLookUp(1, 256, overlayOutputImage.type()); 
        ...
        for (int i = 0; i < 256; i++) {
            ...
            overlayPixelLookUp.at<Vec3b>(i)[0] = color; // R
            overlayPixelLookUp.at<Vec3b>(i)[1] = color; // G
            overlayPixelLookUp.at<Vec3b>(i)[2] = color; // B
        }

        LUT(overlayOutputImage, overlayPixelLookUp, overlayOutputImage);
        /*Added Alt*/
